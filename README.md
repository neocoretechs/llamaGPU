...
```bash
java --enable-preview --source 21 --add-modules jdk.incubator.vector LlamaGPU.java -i --model Meta-Llama-3-8B-Instruct-Q4_0.gguf
```

#### Optional: Makefile + manually build and run

A simple [Makefile](./Makefile) is provided, run `make` to produce `llamaGPU.jar` or manually:
```bash
javac -g --enable-preview -source 21 --add-modules jdk.incubator.vector -d target/classes LlamaGPU.java
jar -cvfe llamaGPU.jar com.llama4j.LlamaGPU LICENSE -C target/classes .
```

Run the resulting `llamaGPU.jar` as follows: 
```bash
java --enable-preview --add-modules jdk.incubator.vector -jar llamaGPU.jar --help
```
...
```bash
native-image -H:+UnlockExperimentalVMOptions -H:+VectorAPISupport -H:+ForeignAPISupport -O3 -march=native --enable-preview --add-modules jdk.incubator.vector --initialize-at-build-time=com.llama4j.FloatTensor -Djdk.incubator.vector.VECTOR_ACCESS_OOB_CHECK=0 -jar llamaGPU.jar -o llamaGPU
```
